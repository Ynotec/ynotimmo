...
export default class BookingService {
  ...
  static async createBooking(bookingdata: IBooking): Promise<any> {
    try {
      const response = await fetch(`${BookingService.baseUrl}/create`, {
        method: 'POST',
        headers: {
  ...
  static async trackBooking(email: string): Promise<any> {
    try {
      const response = await fetch(`${BookingService.baseUrl}/track`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error(`Error tracking booking: ${response.statusText}`)
      }
  ...
  static async cancelBooking(cancelData: {
    ...
    try {
      const response = await fetch(`${BookingService.baseUrl}/order/cancel`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
