...
export default class OdooApi {
  ...
  public async authenticate(): Promise<void> {
    ...
    try {
      ...
      const sessionCookieLine = cookies.find((c) => c.startsWith('session_id='))
  ...
  public async verifyCredentials(
    ...
    const methodName = 'verifyCredentials'
    try {
      this.logger.debug(this.serviceName, methodName, 'START')

      // authenticate(db, credential, user_agent_env)
      const args = [[], this.dbName, { login, password, type: 'password' }, {}]

      const uid = await this.call('res.users', 'authenticate', args)

      this.logger.info(
        this.serviceName,
        methodName,
        `User ${login} authenticated successfully with UID: ${JSON.stringify(uid)}`
      )
      return uid
    } catch (error) {
      this.logger.warn(
    ...
    try {
      ...
      if (response.data.error) {
        this.logger.error(
          this.serviceName,
          methodName,
          `Odoo returned an error: ${JSON.stringify(response.data)}`
        )
        throw new Error(
          `Odoo Error: ${JSON.stringify(response.data.error.message) || 'Unknown error'}`
        )
      }
